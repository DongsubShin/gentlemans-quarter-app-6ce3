...
const DashboardPage = () => {
    const { data: stats } = (0, useBookingData_1.useAdminStats)();
    ...
    return (<div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {statCards.map((stat) => (<div key={stat.label} className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
            <div className="flex items-center justify-between mb-4">
              <div className={cn("p-2 rounded-lg text-white", stat.color)}>
                <react_2.Icon icon={stat.icon} className="text-xl"/>
              </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {queue?.map((entry, idx) => (<tr key={entry.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right">
                    <button className="text-slate-400 hover:text-primary transition-colors">
                      <react_2.Icon icon="lucide:more-vertical"/>
                    </button>
                  </td>
                </tr>))}
            </tbody>
